...
const main = module.exports = (searchString, options, callback) => { // eslint-disable-line consistent-return
  ...
  UTIL.getPage(currentRef ? UTIL.buildFromNextpage(currentRef) : UTIL.buildLink(searchString), options.requestParams, (err, body) => { // eslint-disable-line consistent-return, max-len
...
const getFilters = main.getFilters = (searchString, callback) => { // eslint-disable-line consistent-return
  ...
  UTIL.getPage(queryString, options.requestParams, (err, body) => { // eslint-disable-line consistent-return
    if (err) return callback(err);
    let content;
    try {
      const parsed = JSON.parse(body);
